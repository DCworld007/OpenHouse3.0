...
export function setupTestAuth(): void {
  ...
  // This is a test token with a long expiry - only for development
  ...
  // Set cookies for both token formats
  document.cookie = `token=${testToken}; path=/; SameSite=Lax`;
  document.cookie = `auth_token=${testToken}; path=/; SameSite=Lax`;
  
  console.log('Test auth set up with token:', testToken);
