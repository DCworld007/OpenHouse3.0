// Simplified login endpoint for Cloudflare Functions
export async function onRequestPost(context) {
  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        return new Response(JSON.stringify({ error: 'Failed to verify Google token' }), {
          status: 401,
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      return new Response(JSON.stringify({ error: 'Error verifying Google token' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    // Just encode the session data as base64
